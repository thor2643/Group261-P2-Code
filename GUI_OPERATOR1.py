...
Dispensor_number=[5,6,7,8,9,10,11,12] #0 PCB, 1 fuses, 2-4: top cover colors, 5-7: bot cover colors.
...
top_cov_white_entry.place(x=905,y=475)
...
bot_cov_white_entry.place(x=355,y=475)
...
#funktions:
def Update_Entry():
    PCB_entry = tk.Entry(page_2, width=10,highlightthickness=2, highlightbackground="black")
    PCB_entry.insert(0, 0)
    PCB_entry.place(x=115,y=170)
    fuse_entry = tk.Entry(page_2, width=10,highlightthickness=2, highlightbackground="black")
    fuse_entry.insert(0, 0)
...
def Calculate(indput):
    ...
    if indput==1:
        for i in range(0,7):
            print('nummer')
            print(i)
            print('Dispensor numb før')
            print(Dispensor_number[i])
            print('entry value')
            print(entry_values[i])
            Dispensor_number[i]=Dispensor_number[i] + entry_values[i]
            print('Dispensor numb efter')
            print(Dispensor_number[i])
...
def Check_If_Refill_Needed():
    ...
    if Dispensor_number[1] < 41:
        ...
        Fuse_refill=tk.Label(Fuse_frame, text='Refill needed!!', font=('Times New Roman',30,'bold'),fg='red', bg= 'white')
        Fuse_refill.pack()
        Fuse_refill.pack_forget()
    for i in range(2,4):
        if Dispensor_number[i] < 4:
            top_cov_refill=tk.Label(top_cov_frame, text='Refill needed!!', font=('Times New Roman',30,'bold'),fg='red', bg= 'white')
            top_cov_refill.pack()
        else:
            top_cov_refill=tk.Label(top_cov_frame, text='Refill needed!!', font=('Times New Roman',30,'bold'),fg='red', bg= 'white')
            top_cov_refill.pack()
            top_cov_refill.pack_forget()
    for i in range(5,7):
        if Dispensor_number[i] < 4:
            bot_cov_refill=tk.Label(bot_cov_frame, text='Refill needed!!', font=('Times New Roman',30,'bold'),fg='red', bg= 'white')
            bot_cov_refill.pack()
